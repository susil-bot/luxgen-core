...
const notificationSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: [
      'poll_created', 'poll_response', 'poll_completed', 'poll_expired',
      'user_registered', 'user_invited', 'user_role_changed',
      'system_alert', 'security_alert', 'maintenance_notice',
      'subscription_expiring', 'subscription_expired', 'usage_limit_warning',
      'feedback_received', 'comment_added', 'mention',
  ...
  action: {
    type: {
      type: String,
      enum: ['navigate', 'open_url', 'api_call', 'dismiss'],
      default: 'navigate'
    },
    url: String,
    route: String,
  ...
  relatedResource: {
    type: {
      type: String,
      enum: ['poll', 'user', 'tenant', 'session', 'audit_log'],
      trim: true
    },
    id: {
      type: mongoose.Schema.Types.ObjectId,
...
notificationSchema.virtual('deliveryStatus').get(function() {
  if (this.delivery.failedAt) return 'failed';
  if (this.delivery.deliveredAt) return 'delivered';
  if (this.delivery.sentAt) return 'sent';
  if (this.delivery.scheduledAt) return 'scheduled';
  return 'pending';
});
...
notificationSchema.methods.retryDelivery = async function() {
  if (this.canRetry) {
    ...
    this.delivery.failedAt = null;
    this.delivery.sentAt = null;
...
notificationSchema.statics.findUnreadByUser = function(userId) {
  return this.find({ userId, isRead: false, isArchived: false }).sort({ createdAt: -1 });
};
