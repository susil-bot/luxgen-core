...
class DatabaseSetup {
  ...
  async initialize() {
    try {
      console.log('🚀 Initializing database setup with flagging system...');
  ...
  async connectToDatabase() {
    try {
      ...
      const mongoUri = this.dbConfig.getConnectionString();
      ...
      console.log(`✅ MongoDB connected successfully to ${this.dbConfig.isLocal() ? 'Local' : 'Atlas'}`);
    } catch (error) {
  ...
  async createIndexes() {
    try {
      ...
      console.log('Indexes will be created automatically by Mongoose schemas');
