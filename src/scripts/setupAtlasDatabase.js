...
/**
 * MongoDB Atlas Database Setup Script for LuxGen Platform
 * 
 * HIGH PRIORITY RULE: Use MongoDB Atlas ONLY - NO local MongoDB or Docker
 * Following LuxGen rules: Multi-tenant architecture, max 5 records per model
 * 
 * Usage: node src/scripts/setupAtlasDatabase.js
 */
...
if (!MONGODB_URI) {
  ...
  console.log('   export MONGODB_URI="mongodb+srv://username:password@cluster.mongodb.net/luxgen"');
  console.log('');
  process.exit(1);
}
...
if (!MONGODB_URI.includes('mongodb+srv://') && !MONGODB_URI.includes('mongodb://')) {
  ...
  console.log('   Must be: mongodb+srv://username:password@cluster.mongodb.net/luxgen');
  process.exit(1);
}
...
async function setupAtlasDatabase() {
  try {
    console.log('🚀 LuxGen MongoDB Atlas Database Setup');
    console.log('=====================================');
    console.log('HIGH PRIORITY: Using MongoDB Atlas ONLY (no local MongoDB or Docker)');
    ...
    console.error('❌ MongoDB Atlas database setup failed:', error.message);
    throw error;
  } finally {
...
if (require.main === module) {
  setupAtlasDatabase()
    .then(() => {
      console.log('\n🎉 MongoDB Atlas database setup completed successfully!');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ MongoDB Atlas database setup failed:', error);
      process.exit(1);
    });
}
