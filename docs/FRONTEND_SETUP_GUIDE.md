...
- **API Documentation**: `http://192.168.1.9:3001/docs`
...
const API_BASE = 'http://192.168.1.9:3001/api/v1';
...
### JWT Token Usage
After successful registration, you'll receive a JWT token. Use it for authenticated requests:
...
### Test Script
You can test the API connection using this script:

```javascript
// Test the health endpoint
fetch('http://192.168.1.9:3001/health')
...
const testRegistration = async () => {
  try {
    const response = await fetch('http://192.168.1.9:3001/api/v1/registration/register', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
If you're running on a different port, contact the backend developer to add it to the CORS configuration.
...
3. **Implement in Frontend**:
   ```javascript
   const response = await apiClient.post('/registration/register', formData);
   ```
...
4. Contact the backend developer with error details
