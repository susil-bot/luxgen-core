...
The AI module provides powerful content generation capabilities using Groq API with RAG (Retrieval-Augmented Generation) support. This module enables trainers to generate various types of content including training materials, assessments, feedback templates, and more.

## Features

- 🤖 **Content Generation**: Generate text content using Groq's LLM models
...
2. **OpenAI API Key**: Get from [OpenAI Platform](https://platform.openai.com/) (for embeddings)
...
```json
{
  ...
  "data": {
    ...
    "knowledgeBaseSize": 5,
    "groqHealthy": true,
    "timestamp": "2025-07-26T21:40:24.047Z"
  },
...
```json
{
  "prompt": "Create a training module about customer service",
  ...
  "presencePenalty": 0.0,
  "systemPrompt": "You are an expert training content creator",
  "useRAG": true
}
```
...
**POST** `/generate/specialized`

Generate content for specific training use cases.
...
```json
{
  ...
  "data": {
    "success": true,
    "content": "Specialized content here...",
    "model": "llama-3.3-70b-versatile",
    "usage": {
      "prompt_tokens": 75,
    ...
    "timestamp": "2025-07-26T21:40:24.047Z",
    "type": "training_material",
    "originalPrompt": "Customer service best practices"
  },
  "message": "training_material content generated successfully"
}
```
...
```json
{
  "documentId": "customer-service-guide-2024",
  "content": "Complete customer service training guide content...",
  "metadata": {
    "title": "Customer Service Best Practices 2024",
...
```json
{
  ...
  "data": {
    "documentId": "customer-service-guide-2024",
    "chunksCount": 8,
    "message": "Document added to knowledge base with 8 chunks"
  }
}
```
...
```json
{
  "query": "How to handle difficult customers?",
  "maxResults": 5
}
```
...
```json
{
  ...
  "data": {
    ...
    "results": [
      {
        "chunkId": "customer-service-guide-2024_chunk_3",
        "similarity": 0.85,
        "content": {
...
async function generateTrainingContent() {
  try {
    const response = await axios.post(`${API_BASE}/generate/specialized`, {
      type: 'training_material',
      prompt: 'Effective communication skills for managers',
      options: {
        maxTokens: 2048,
        temperature: 0.7
      }
...
async function addToKnowledgeBase() {
  try {
    const response = await axios.post(`${API_BASE}/knowledge-base/add`, {
      documentId: 'manager-guide',
      content: 'Complete manager training guide...',
      metadata: {
    ...
    console.log('Added to knowledge base:', response.data.data);
  } catch (error) {
    console.error('Error:', error.response?.data || error.message);
  }
}
...
curl -X POST http://localhost:3001/api/v1/ai/generate/specialized \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST http://localhost:3001/api/v1/ai/knowledge-base/add \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "documentId": "leadership-guide",
    "content": "Leadership training content...",
    "metadata": {
...
1. **Use Specialized Endpoints**: Use `/generate/specialized` for training-specific content
...
6. **Content Validation**: Validate generated content before using in production
...
1. **"Groq client not available"**
   - Check if `GROQ_API_KEY` is set in environment
   - Verify API key is valid

2. **"RAG features disabled"**
